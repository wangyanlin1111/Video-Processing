...
class IndexTTS2:
    ...
    async def sleep(self, level: int = 1) -> None:
        """
        休眠 vLLM 引擎，释放 GPU 显存。唤醒后可继续推理。
        level=1: 清除 MM cache + 暂停调度器 (推荐)
        level=2: 深度休眠
        """
        logger.info(f"Sleeping GPT engine (level={level})...")
        await self.gpt.llm.sleep(level=level)
        if self.qwen_emo is not None:
            logger.info(f"Sleeping QwenEmotion engine (level={level})...")
            await self.qwen_emo.model.sleep(level=level)
        logger.info("All engines sleeping.")

    async def wake_up(self, tags: list = None) -> None:
        """从休眠中唤醒 vLLM 引擎"""
        logger.info("Waking up GPT engine...")
        await self.gpt.llm.wake_up(tags=tags)
        if self.qwen_emo is not None:
            logger.info("Waking up QwenEmotion engine...")
            await self.qwen_emo.model.wake_up(tags=tags)
        logger.info("All engines awake.")

    async def reset_engine_cache(self) -> None:
        """清除 vLLM 引擎的 KV cache 和 MM cache (不进入休眠)"""
        logger.info("Resetting GPT engine cache...")
        await self.gpt.llm.reset_mm_cache()
        await self.gpt.llm.reset_prefix_cache()
        if self.qwen_emo is not None:
            logger.info("Resetting QwenEmotion engine cache...")
            await self.qwen_emo.model.reset_mm_cache()
            await self.qwen_emo.model.reset_prefix_cache()
        logger.info("Engine caches reset.")
    ...
    async def infer(self, spk_audio_prompt, text, output_path,
        ...
        # 从 generation_kwargs 提取语速控制参数
        # speed = 1.0 正常语速, speed = 0.5 语速加倍/时长减半, speed = 2.0 语速减半/时长加倍
        # 通过缩放 length_regulator 的 target_lengths 实现自适应语速调节
        speed = generation_kwargs.pop("speed", 1.0)
        # max_mel_tokens 作为硬上限 (None 表示不限制)
        max_mel_tokens = generation_kwargs.pop("max_mel_tokens", None)
        # reset 参数用于控制是否在每次推理前重置 vLLM 引擎的参考音频相关特征
        ref_reset = generation_kwargs.pop("ref_reset", 1)
        emo_reset = generation_kwargs.pop("emo_reset", 1)

        if use_emo_text:
            if self.qwen_emo is None:
                raise RuntimeError(
                    "QwenEmotion model not loaded. Set load_qwen_emo=True or use emo_vector instead."
                )
        ...
        if ref_reset == 1:
            ...
            self.spk_cond_emb = self.get_emb(input_features, attention_mask)

            _, S_ref = self.semantic_codec.quantize(self.spk_cond_emb)
            self.ref_mel = self.mel_fn(audio_22k.to(self.spk_cond_emb.device).float())
            ref_target_lengths = torch.LongTensor([self.ref_mel.size(2)]).to(self.ref_mel.device)
            feat = torchaudio.compliance.kaldi.fbank(audio_16k.to(self.ref_mel.device),
                                                        num_mel_bins=80,
                                                        dither=0,
                                                        sample_frequency=16000)
            feat = feat - feat.mean(dim=0, keepdim=True)  # feat2另外一个滤波器能量组特征[922, 80]
            self.style = self.campplus_model(feat.unsqueeze(0))  # 参考音频的全局style2[1,192]

            self.prompt_condition = self.s2mel.models['length_regulator'](S_ref,
        ...
        if emo_ref == 1:
            ...
            self.emo_cond_emb = self.get_emb(emo_input_features, emo_attention_mask)
        ...
        if emo_vector is not None:
            ...
            if use_random:
                ...
                random_index = [find_most_similar_cosine(self.style, tmp) for tmp in self.spk_matrix]
        ...
        for sent in sentences:
            ...
            with torch.no_grad():
                emovec = self.gpt.merge_emovec(
                    self.spk_cond_emb,
                    self.emo_cond_emb,
                    torch.tensor([self.spk_cond_emb.shape[-1]], device=text_tokens.device),
                    torch.tensor([self.emo_cond_emb.shape[-1]], device=text_tokens.device),
                    alpha=emo_alpha
                )
                ...
                codes, speech_conditioning_latent = await self.gpt.inference_speech(
                    ...
                    text_tokens,
                    self.emo_cond_emb,
                    cond_lengths=torch.tensor([self.spk_cond_emb.shape[-1]], device=text_tokens.device),
                    emo_cond_lengths=torch.tensor([self.emo_cond_emb.shape[-1]], device=text_tokens.device),
                    emo_vec=emovec,
                ...
                m_start_time = time.perf_counter()
                use_speed = torch.zeros(self.spk_cond_emb.size(0)).to(self.spk_cond_emb.device).long()
                ...
                latent = self.gpt(
                    ...
                    self.emo_cond_emb,
                    cond_mel_lengths=torch.tensor([self.spk_cond_emb.shape[-1]], device=text_tokens.device),
                    emo_cond_mel_lengths=torch.tensor([self.emo_cond_emb.shape[-1]], device=text_tokens.device),
                    emo_vec=emovec,
                    use_speed=use_speed,
                )
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    # speed 缩放 target_lengths 实现语速调节
                    # speed=1.0 → 正常, speed=0.5 → 加倍语速, speed=2.0 → 减半语速
                    ...
                    vc_target = self.s2mel.models['cfm'].inference(cat_condition,
                                                                   torch.LongTensor([cat_condition.size(1)]).to(
                                                                       cond.device),
                                                                   self.ref_mel, self.style, None, diffusion_steps,
